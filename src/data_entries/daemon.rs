...
pub async fn start<T: DataEntriesSource + Send + Sync, U: DataEntriesRepo>(
    ...
    while let Ok(_) = result {
        // nested pattern matching is cause unstable async/await in closures
        match dbw.get_last_handled_height() {
            Ok(last_handled_height) => {
                ...
                result = match updates_src.fetch_updates(from_height, to_height).await {
                    Ok(updates_with_height) => {
                        info!(
                            APP_LOG,
                        ...
                        dbw.transaction(|conn| {
                            ...
                            match updates_with_height
                                    ...
                                    BlockchainUpdate::Block(b) => {
                                        ...
                                        if acc.len() > 0 {
                                            match acc.iter_mut().nth(len as usize - 1).unwrap() {
                                                ...
                                                UpdatesItem::Microblock(_)
                                                | UpdatesItem::Rollback(_) => {
                                ...
                                .fold(Ok(()), |_, update_item| match update_item {
                                    UpdatesItem::Blocks(bs) => squash_microblocks(dbw.clone())
                                        .and_then(|_| {
                                            append_blocks_or_microblocks(dbw.clone(), bs.as_ref())
                                        }),
                                    UpdatesItem::Microblock(mba) => squash_microblocks(dbw.clone())
                                        .and_then(|_| {
                                    ...
                                    UpdatesItem::Rollback(sig) => {
                                        match dbw.try_lock().unwrap().get_block_uid(&sig) {
                                            Ok(block_uid) => dbw
                                                .lock()
                                                .unwrap()
                                                .rollback_blocks_microblocks(&block_uid),
                                            Err(err) => Err(err),
                                        }
                                    }
                                }) {
...
fn append_blocks_or_microblocks<U: DataEntriesRepo>(
    ...
    dbw.insert_blocks_or_microblocks(
        &appends
            .into_iter()
    ...
    dbw.get_last_update_uid()
        .and_then(|last_uid| {
                ...
                .map(
                    |(
                        ...
                        let frs: Vec<&str> = key
                            ...
                            .map(|fr| RE.captures(fr).unwrap().get(1).unwrap().as_str())
                            .collect();
            ...
            dbw.close_superseded_by(&first_uids)
                ...
                .and_then(|_| {
                    dbw.set_last_update_uid(
                        last_uid + updates_with_uids_superseded_by.len() as i64 + 1,
                    )
                })
        })
        .map(|_| ())
...
fn squash_microblocks<U: DataEntriesRepo>(dbw: Arc<Mutex<U>>) -> Result<(), Error> {
    dbw.try_lock()
        .unwrap()
        .get_total_block_id()
        .and_then(|total_block_id| match total_block_id {
            Some(total_block_id) => {
                dbw.try_lock()
                    .unwrap()
                    .get_key_block_uid()
                    .and_then(|key_block_uid| match key_block_uid {
                        Some(key_block_uid) => dbw
                            .lock()
                            .unwrap()
                            .update_data_entries_block_references(&key_block_uid)
                            .and_then(|_| dbw.try_lock().unwrap().delete_microblocks())
                            .and_then(|_| {
                                dbw.try_lock()
                                    .unwrap()
